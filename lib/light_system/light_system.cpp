...
void LSystem::calculateLineDirection(float rot) {
    ...
    // if(sensorIsWhite[0] || sensorIsWhite[1] || sensorIsWhite[31]) {
    //     sensorIsWhite[0] = 1;
    //     sensorIsWhite[1] = 1;
    //     sensorIsWhite[31] = 1;
    // }
    ...
    calculateLineState(rot, lineDirection);
}
...
void LSystem::calculateLineState(float rot, float lineDirection) {
    ...
    float relLineDirection = imOnLine ? floatMod(lineDirection + rot, 360.0) : -1;
    if(lineState == 0) {
        if(imOnLine) {
            lineState = 1;
